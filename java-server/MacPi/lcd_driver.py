...
def w(message, lineNo):
	lcd_string(message, LCD_LINES[lineNo], 1)

def c(char, lineNo, col):
	if col >= LCD_WIDTH:
		syso("col not available: " + str(col))
	lcd_byte(LCD_LINES[lineNo] + col, LCD_CMD)
	lcd_byte(ord(char[0]), LCD_CHR)
